...
export default class ActionContainer extends Vue {
  ...
  onMouseUp () : void {
    ...
    if (this.actionMouseButton.button != null &&
  ...
  onMouseMove (e:MouseEvent) : void {
    this.cursor.new = this.getPosition(e)
    if (this.actionMouseButton.button == null || this.actionMouseButton.button > 2) {
  ...
  emitMouseAction (actionName: string) : void {
    ...
    actionCursorEvent.new = this.cursor.new == null
      ? null
      : this.transforming(this.cursor.new)
    actionCursorEvent.last = this.cursor.last == null
      ? null
      : this.transforming(this.cursor.last)
    actionCursorEvent.start = this.cursor.start == null
      ? null
      : this.transforming(this.cursor.start)
    const actionData = new ActionCallbackData(
      this.transform,
      this.scale,
      actionCursorEvent
  ...
  zoomInPosition (zoom: number, position: V) : void{
    ...
    const delta = this.scaling(this.transform.size).multN(zoom).multV(ratio)
    this.transform.position = this.transform.position.addV(delta)
  }
