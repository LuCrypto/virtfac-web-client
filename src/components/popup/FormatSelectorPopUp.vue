      ...
      <div class="pa-6">
        ...
        >
          <template slot="item" slot-scope="{ item }">
            <div :class="{ 'null-list-item': item.id === 'NULL' }">
              {{ item.text }}
            </div>
          </template>
        ...
        <div>
          The following figure shows a schematic view of the selected data
          format :
        </div>
        <format-preview
...
export default class FormatSelectorPopUp extends Vue {
  ...
  availableFormatList: FormatInfo[] = Object.keys(FORMAT_INFO).map(
    key => FORMAT_INFO[key as FORMAT_TYPE]
  )
  ...
  open (id: string): void {
    this.selectedFormat =
      this.availableFormatList.filter(format => format.id === id).pop() ||
      this.availableFormatList[0]
    this.show = true
  }
